...
export default async function MorningSkateEditionPage({
  params,
}: {
  params: Promise<{ leagueId: string; editionId: string }>;
}) {
  const { leagueId, editionId } = await params;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 680 }}>
      <div>
        <Link href={`/league/${leagueId}/morning-skate`} style={{
