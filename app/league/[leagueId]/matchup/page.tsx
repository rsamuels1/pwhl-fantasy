...
export default async function CurrentMatchupPage({
  params,
  searchParams,
}: {
  ...
  const league = await prisma.fantasyLeague.findUnique({
    ...
    select: {
      ...
      teams: { select: { id: true, name: true, ownerId: true } },
    },
  });
  if (!league) notFound();

  const myTeam =
    (searchParams.team ? league.teams.find((t) => t.id === searchParams.team) : null) ??
    league.teams.find((t) => t.ownerId === user?.id) ??
    league.teams[0];

  if (!myTeam) {
    return (
      <div style={{ color: "#94a3b8", padding: 32 }}>No teams in this league yet.</div>
    );
  }

  const scoringSettings = parseScoringSettings(league.scoringSettings);
  const dashboard = await getDashboardData(leagueId, myTeam.id, Date.now(), prisma);
  const { activeMatchup, remainingPlayers, topPerformers, disappointments, leagueActivity } = dashboard;

  // Fetch swing players only when there's an active matchup with a real period
      ...
      {league.teams.length > 1 && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {league.teams.map((t) => (
            <a
              key={t.id}
              href={`/league/${leagueId}/matchup?team=${t.id}`}
              style={{
                padding: "6px 14px",
                borderRadius: 999,
                fontSize: 13,
                background: t.id === myTeam.id ? "#6366f1" : "rgba(255,255,255,0.06)",
                color: "#e2e8f0",
                textDecoration: "none",
              }}
            >
    ...
    >
      ...
      >
        <div>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 6 }}>{leadLabel}</h1>
