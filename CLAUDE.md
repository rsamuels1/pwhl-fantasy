...
6. Integration + load test the draft room + beta
   ...
   - **Inviting Dark Redesign — Sprint 22** 🟡 IN PROGRESS (8/23 stories shipped; no schema changes):
     - **BF-018: /league-rules page** ✅ (`app/league-rules/page.tsx` created; public static page, no auth required; five sections — scoring, roster, draft, standings, playoffs; fixes dead links from WelcomeFlow and BetaWelcomeStep)
     - **RD-001: Token Swap** ✅ (`app/globals.css` `:root` block replaced with Inviting Dark / Northern Ice tokens; sky-blue accent `#8fc1e8`; body radial-gradient, `.button-primary` accent-ink text, `.section-accent` drop-violet all updated)
     - **RD-002: Hex Sweep** ✅ (swept old violet `#7c3aed`/`#6366f1`, old green `#5fa98c`/`#7fc2a6`/`#34d399`, old gold `#e3c989`/`#fbbf24`, old red `#c2776c`/`#d18b7f`/`#f87171` across `app/**` + `components/**`; replaced with `var(--green)`, `var(--gold)`, `var(--red)`, `var(--accent)` etc.; `app/founder/` intentionally preserved monochrome; `TeamColorPicker.tsx` + `color/route.ts` intentionally left since indigo is a valid user-selectable team color)
     - **RD-003: Emoji Policy** ✅ (tiered emoji policy applied to activity/transaction feed labels and WeekHighlights award colors; blanket-ban relics removed)
     - **RD-004: VP Popover Fix + Wizard Rebuild** ✅ (`components/VpExplainer.tsx` rebuilt with solid anchored popover, arrow caret, popIn animation, open-state filled accent `?` button, outside-click/Esc close; `app/create-league/CreateLeagueWizard.tsx` Rules step replaced with proper rule-sheet card: roster pills, two-column scoring table, labeled Standings/Playoffs/Season sections with `VpExplainer` inline; covers UX-051 mobile overflow fix and UX-057 PPP/UTIL inline definitions)
     - **RD-007: Remaining Page Recolor Sweep** ✅ (swept all remaining hardcoded hex values from `app/page.tsx`, `app/leagues/page.tsx`, `app/beta/page.tsx`, `app/join-league/page.tsx`, `components/MatchupTabs.tsx`, `app/league/[leagueId]/layout.tsx`, `SettingsEditor.tsx`, `DraftRoom.tsx`, `app/league/[leagueId]/page.tsx`, `app/team/[teamId]/matchup/page.tsx`, `app/league/[leagueId]/trades/TradeCenter.tsx`)
     - **RD-008: MomentumStrip** ✅ (`components/MomentumStrip.tsx` created; delta chip (green ▲ / red ▼), "⚡ N playing tonight" accent chip, "Opponent done" dim chip; returns null when no chips; wired into matchup page inside `status === "active"` block above Z3 two-column grid; completes LL-002)
     - **UX-051: VP Popover Mobile Fix** ✅ (fixed via RD-004; `position:relative` wrapper prevents popover from clipping viewport)
     - **UX-052: Invite Landing Primer** ✅ (`app/invite/[leagueId]/page.tsx` updated with "New to fantasy sports?" section; four two-level steps with bold heading + plain-language sub-copy)
     - **UX-057: Wizard Jargon Fix** ✅ (fixed via RD-004 wizard rebuild; PPP and UTIL get inline definitions in the scoring table)
     - Remaining: RD-005 (league overview flagship) · RD-006 (matchup flagship) · RD-009–012 P2 emotional design · RD-014–017 · UX-054/055/056
